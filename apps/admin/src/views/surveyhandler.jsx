...
function SurveyHandler(props) {
  ...
  const [showConfirmDialogOverwrite, setShowConfirmDialogOverwrite] =
    useState(false);
  ...
  const [alertDialogMessageNotSaved, setAlertDialogMessageNotSaved] =
    useState("");
  ...
  const renderSelectedQuestionForm = () => {
    if (
      ...
      !survey.pages[selectedQuestion.pageIndex]
    ) {
      return null;
    }
    const question =
      survey.pages[selectedQuestion.pageIndex].questions[
      ...
      <div>
        ...
        <div>
          ...
          >
            ...
            <MenuItem value="geometrylinestring">
              Geometriverktyget linje
            </MenuItem>
            <MenuItem value="geometrypolygon">Geometriverktyget yta</MenuItem>
            <MenuItem value="geometrypointposition">
              Geometriverktyget punkt + min position
            </MenuItem>
          </Select>
        </div>
  ...
  const deleteQuestion = (pageIndex, questionIndex) => {
    ...
    if (
      selectedQuestion &&
      selectedQuestion.pageIndex === pageIndex &&
  ...
  const loadSurvey = (surveyId) => {
    ...
    setSelectedSurveyIdOld(surveyId);
    if (surveyId) {
      props.model.loadSurvey(surveyId, (surveyData) => {
        setSurvey(surveyData);
        setFilename(surveyId);
      });
    }
  };
  ...
  const GeometryWarning = ({ survey }) => {
    ...
    useEffect(() => {
      const pagesWithMultipleGeometry = survey.pages.reduce(
        (acc, page, pageIndex) => {
    ...
    return (
      ...
      >
        <strong>Varning:</strong> Följande sidor innehåller mer än en
        geometri-fråga: sida {warningPages.join(", ")}
      </div>
    );
  };
  ...
  return (
    <div>
      ...
      <Grid container spacing={2} style={{ marginBottom: "20px" }}>
        ...
        <Grid item>
          <Button variant="contained" color="primary" onClick={newSurvey}>
            Töm fält
          </Button>
        </Grid>
      </Grid>
                ...
                >
                  ...
                  {page.questions.map((question, questionIndex) => (
                    <p
                      key={questionIndex}
                      onClick={() =>
                        handleQuestionClick(pageIndex, questionIndex)
                      }
                    >
                      {question.title
                        ? `${question.title} (${
          ...
          <div style={{ backgroundColor: "#f0f0f0", padding: "20px" }}>
            <div>
              ...
              <Select
                ...
                onChange={(e) =>
                  handlePageSelection(parseInt(e.target.value, 10))
                }
              >
              ...
              <Button
                variant="contained"
                color="primary"
                onClick={() => addQuestion(selectedPageIndex)}
      ...
      {showConfirmDialogValid && (
        <Dialog open={showConfirmDialogValid} onClose={handleDialogAbortValid}>
          ...
          <DialogContent>
            Ogiltigt filnamn. Endast bokstäver, siffror och understreck är
            tillåtet.
          </DialogContent>
          <DialogActions>
      ...
      {showConfirmDialogOverwrite && (
        <Dialog
          open={showConfirmDialogOverwrite}
      ...
      {showAlertDialog && (
        <Dialog open={showAlertDialog} onClose={handleCloseAlertDialog}>
          ...
          <DialogContent>{alertDialogMessage}</DialogContent>
          <DialogActions>
      ...
      {showAlertDialogNotSaved && (
        <Dialog
          open={showAlertDialogNotSaved}
          onClose={handleCloseAlertDialogNotSaved}
        >
          <DialogTitle>Meddelande</DialogTitle>
          <DialogContent>{alertDialogMessageNotSaved}</DialogContent>
          <DialogActions>
            <Button
              onClick={handleCloseAlertDialogNotSaved}
              color="primary"
              autoFocus
            >
            ...
            <Button
              onClick={handleContinueAlertDialogNotSaved}
              color="primary"
              autoFocus
            >
